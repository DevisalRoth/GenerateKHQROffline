...
+(KHQRResponse *)verify:(NSString *)khqrCode {
    @try {
        ...
        NSArray * sortKeys = [[taglist allKeys] sortedArrayUsingComparator:^NSComparisonResult(NSString *key1, NSString *key2) {
        ...
        for(NSString* tag in sortKeys) {
            ...
            if ([tag isEqualToString: @"00"]) {
                ...
                trAmount = [[TransactionAmount alloc] initWithAmount: 0 currencyType: crType];
        ...
        if ([pointOfInit isEqualToString: @"12"]) {
            if (!trAmount.value) {
                KHQRException * requiredException = [[KHQRException alloc] initWithReason: invalidDynamicKhqr
...
+(KHQRResponse *)decodeNonKhqr: (NSString *)qr {
    ...
    @catch (KHQRException *exception) {
        if (exception.errorCode == ErrorConstants.qrInvalidCode) {
            // Error mapping from invalid KHQR to invalid EMVQR
            return [[KHQRResponse alloc] initWithErrorCode: ErrorConstants.invalidEMVQRCode
                                                   message: ErrorConstants.invalidEMVQRMessage
                                                      data: NULL];
        }
        return [[KHQRResponse alloc] initWithErrorCode: exception.errorCode
                                               message: exception.message
...
- (KHQREMVQRData *) internalDecodeNonKhqr: (NSString* )qr {
    ...
    @try {
        ...
        for(NSString* tag in dataDictionary) {
            ...
            if (
                (tag.intValue >= 26 && tag.intValue <= 51) == FALSE &&
                (tag.intValue >= 80 && tag.intValue <= 99) == FALSE &&
                (tag.intValue == 64) == FALSE &&
                (tag.intValue == 62) == FALSE
               ) {
                continue;
            }
            ...
            if ([value length] >= 6) {
                do {
                    ...
                    remaningValue = [Constants replaceString:remaningValue withEmptyStringToIndex: 2];
                    
                    NSString * length = [Constants subString:remaningValue toIndex: 2];
                    remaningValue = [Constants replaceString: remaningValue withEmptyStringToIndex: 2];
                    
                    NSString * value = [Constants subString:remaningValue toIndex: length.intValue];
                    remaningValue = [Constants replaceString: remaningValue withEmptyStringToIndex: length.intValue];
                    
                    // level 3
                    if (tag.intValue == 62 && (subTag.intValue >= 50 && subTag.intValue <= 99)) {
                        // decode as dictionary for main tag is 62 and subtags range from 50-99
                        NSString * remaningValueL3 = value;
                        do {
                            NSString * subTagL3 = [Constants subString:remaningValueL3 toIndex: 2];
                            // check numberic
                            if ([Constants isNumberic:subTagL3] == FALSE) {
                                break;
                            }
                            remaningValueL3 = [Constants replaceString:remaningValueL3 withEmptyStringToIndex: 2];
                            
                            NSString * lengthL3 = [Constants subString:remaningValueL3 toIndex: 2];
                            remaningValueL3 = [Constants replaceString: remaningValueL3 withEmptyStringToIndex: 2];
                            
                            NSString * valueL3 = [Constants subString:remaningValueL3 toIndex: lengthL3.intValue];
                            remaningValueL3 = [Constants replaceString: remaningValueL3 withEmptyStringToIndex: length.intValue];
    ...
    @try {
        ...
        if (info.amount) {
            if (info.expirationTimestamp != nil) {
                ...
                NSString * expirationTimestampString = [NSString stringWithFormat: @"%.0f", expirationTimestampDouble];
                ReserveForUse * reserveForUse = [[ReserveForUse alloc] initWithMilliseconds: expirationTimestampString];
